...
export default function FieldsetForm({ name, type, labelText, value, onChange, errorMessages, children, required=false } : { 
    name: string, 
    ...
    return (
        <fieldset className={`fieldset ${errorMessages.length > 0 && 'fieldset-error'} ${value && errorMessages.length < 1 && 'fieldset-valid'}`}>
            <input id={'editor-' + name} name={name} type={type} placeholder="" value={value} required={required} onChange={(e: ChangeEvent<HTMLInputElement>) => {
